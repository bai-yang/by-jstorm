...
import com.a.eye.by.jstorm.utils.Assert;
import com.a.eye.by.jstorm.utils.JStormHelper;
import com.alibaba.jstorm.utils.JStormUtils;

/**
 * WordCount but teh spout does not stop, and the bolts are implemented in java. This can show how fast the word count
 * can run.
 */
public class FastWordCountTopology {
    ...
    public static class FastRandomSentenceSpout implements IRichSpout {
        ...
        public void nextTuple() {
            ...
            /*
             * while (--n >= 0) { String sentence = CHOICES[_rand.nextInt(CHOICES.length)]; _collector.emit(new
             * Values(sentence)); }
             */

            for (int i = 0; i < CHOICES.length; i++) {
                String sentence = CHOICES[i];
    ...
    public static void test() {
        ...
        try {
            JStormHelper.runTopology(builder.createTopology(), topologyName, conf, 60, new JStormHelper.CheckAckedFail(
                    conf), isLocal);
        } catch (Exception e) {
